...
export async function listZones(query = {}) {
    ...
    const [docs, total] = await Promise.all([
        TaxiZone.find(filter)
            .sort(sort)
            .skip(parsed.skip)
            .limit(parsed.limit)
            .lean(),
        TaxiZone.countDocuments(filter),
    ]);
...
export async function getZoneById(id) {
    ...
    const doc = await TaxiZone.findOne({ _id: zoneId, ...baseFilter }).lean();
...
export async function createZone(body, reqUser) {
    ...
    const existing = await TaxiZone.findOne({
        ...baseFilter,
    ...
    const doc = await TaxiZone.create({
        ...payload,
        createdBy: performer,
        updatedBy: performer,
...
export async function updateZone(id, body, reqUser) {
    ...
    const doc = await TaxiZone.findOne({ _id: zoneId, ...baseFilter });
    if (!doc) throw new NotFoundError('Zone not found');
...
export async function listZoneDropdown() {
    const docs = await TaxiZone.find({ ...baseFilter, status: 'active' })
