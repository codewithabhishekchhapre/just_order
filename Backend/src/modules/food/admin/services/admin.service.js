...
function validateCommissionRuleSet(rules) {
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    ...
    const max =
      current.maxDistance == null ? null : Number(current.maxDistance);
    if (max != null && max <= min) {
    ...
    if (i > 0) {
      ...
      const prevMax =
        prev.maxDistance == null ? null : Number(prev.maxDistance);
      ...
      if (min === prevMin) {
        throw new ValidationError(
          "Distance slabs must not share the same minDistance",
        );
      }
    }
  }
...
export async function getZoneHubs(query = {}) {
  ...
  const [zones, total] = await Promise.all([
    FoodZone.find(filter).sort({ name: 1 }).skip(skip).limit(limit).lean(),
    FoodZone.countDocuments(filter),
  ]);
  ...
  const zoneHubsPromises = zones.map(async (z) => {
    const hubs = await FoodRestaurant.find({
      ...
      .select(
        "restaurantName primaryContactNumber ownerPhone ownerName addressLine1 location status city",
      )
      .lean();
      ...
      hub:
        hubs.length > 0
          ? {
              ...
              phone:
                hubs[0].primaryContactNumber || hubs[0].ownerPhone || "N/A",
              owner: hubs[0].ownerName || "N/A",
              address:
                hubs[0].addressLine1 || hubs[0].location?.addressLine1 || "N/A",
...
export async function assignZoneHub(zoneId, restaurantId, action = "assign") {
  ...
  if (String(restaurant.zoneId) !== String(zoneId)) {
    throw new ValidationError(
      "Selected restaurant does not belong to this zone",
    );
  }
  ...
  if (action === "unassign") {
    ...
    if (restaurant.status !== "approved") {
      throw new ValidationError(
        "Only approved restaurants can be designated as Zone Hub",
      );
    }
...
export async function getAdminCODVerifications(query = {}) {
  ...
  const [requests, total] = await Promise.all([
    FoodDeliveryCashDeposit.find(filter)
      ...
      .populate(
        "zoneHubRestaurantId",
        "restaurantName restaurantId ownerName ownerPhone",
      )
      .lean(),
    FoodDeliveryCashDeposit.countDocuments(filter),
  ]);
...
export async function settleCODVerification(
  id,
  { action, adminNote, performer },
) {
  if (!id || !mongoose.Types.ObjectId.isValid(id))
    throw new ValidationError("Invalid request ID");
  ...
  if (deposit.status !== "Restaurant_Accepted") {
    throw new ValidationError(
      `Request cannot be processed because status is: ${deposit.status}`,
    );
  }
