        ...
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <Input 
              ...
              className="pl-10 pr-10 h-11 bg-slate-100 dark:bg-zinc-800 border-none focus:ring-2 focus:ring-[#FF6A00] rounded-xl"
            />
            {query && (
            ...
            <button 
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isListening ? 'text-[#FF6A00] scale-125 animate-pulse' : 'text-slate-400'}`}
            >
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
              {categories.map((cat) => (
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl mb-2 flex items-center justify-center overflow-hidden border-2 transition-all ${selectedCategoryId === cat._id ? 'border-[#FF6A00] shadow-lg shadow-[#FF6A00]/10' : 'border-transparent bg-white dark:bg-zinc-900'}`}>
                    {cat.image ? (
                      <img 
            ...
            {results.dishes.length > 0 && (
              <section>
                <div className="flex items-center gap-2 mb-4">
                   <div className="w-1 h-5 bg-[#FF6A00] rounded-full" />
                  ...
                  {results.dishes.map((r) => (
                    <Link to={`/food/user/restaurants/${r.slug || r._id}${r.matchedDishId ? `?dish=${r.matchedDishId}` : ''}`} key={r._id} className="flex gap-4 p-3 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 hover:shadow-md transition-shadow group">
                       ...
                       <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <div className="text-[#FF6A00] text-[10px] font-bold uppercase tracking-wider mb-1">
                  ...
                  {results.restaurants.map((r) => (
                    <Link to={`/food/user/restaurants/${r.restaurantId || r._id}`} key={r._id} className="block group">
                      <div className="relative rounded-3xl overflow-hidden aspect-[16/9] mb-3 bg-slate-200">
                        ...
                        {r.offer && (
                           <div className="absolute top-4 left-0 bg-[#FF6A00] text-white text-[10px] font-bold px-3 py-1.5 rounded-r-lg shadow-lg flex items-center gap-1 tracking-tighter">
                      ...
                      <div className="flex items-center justify-between px-1">
                         ...
                         <div className="text-[10px] font-bold text-[#FF6A00] bg-[#fff5f5] dark:bg-[#FF6A00]/10 px-2 py-0.5 rounded-full uppercase tracking-wider">
                            Top Pick
                         </div>
                      </div>
                    </Link>
