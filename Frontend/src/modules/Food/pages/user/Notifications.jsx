  ...
  useEffect(() => {
    const handleOrderUpdate = (event) => {
      ...
      const newNotification = {
        ...
        iconColor: isCancelled ? "text-red-600" : "text-[#FF6A00]"
      }
      ...
      <div className="max-w-md md:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-4 md:mb-6 lg:mb-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1">
            <Bell className="h-5 w-5 sm:h-6 sm:w-6 text-[#FF6A00] fill-[#FF6A00]" />
            <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-800 dark:text-white">Notifications</h1>
            {unreadCount > 0 && (
              <Badge className="bg-[#FF6A00] text-white text-xs md:text-sm">
                {unreadCount}
              </Badge>
            )}
          </div>
          ...
          {mergedNotifications.map((notification) => {
            ...
            return (
              ...
              >
                ...
                {!notification.read && (
                  <div className="absolute top-2 right-2 w-2.5 h-2.5 md:w-3 md:h-3 bg-[#FF6A00] rounded-full" />
                )}
