...
export default function TaxiBookingSheet({
  ...
  const isLivePhase = phase === "finding" || phase === "driver" || phase === "trip";

  const fare = Number(
    activeRide?.fareEstimateTotal ?? activeRide?.fare?.total ?? selectedQuote?.fare ?? 0,
  );
  ...
  const liveRideBody =
    (phase === "driver" || phase === "trip") && activeRide ? (
      <div className="space-y-4 pb-2">
        ...
        <div className={`grid gap-2 ${phase === "trip" ? "grid-cols-2" : "grid-cols-1"}`}>
